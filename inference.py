...
def evaluate(model, mt_test):
    ...
    for fold in sorted(os.listdir(config.CHECKPOINT_PATH)):
        ...
        load_checkpoint(
            os.path.join(config.CHECKPOINT_PATH, fold, "model_best.pth.tar"), model,
            optimizer, lr=2e-4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input_path', default=2,
                        help='Input path for inference', required=True, type=str)
    parser.add_argument('-o', '--output_path', default=2,
                        help='Output path for results from inference', required=False, type=str)
